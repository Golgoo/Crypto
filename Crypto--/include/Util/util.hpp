...
namespace operation
{
  void XOR(std::vector<uchar>& dst, std::vector<uchar>& other, int other_start = 0);
  void RotWord(std::vector<uchar> &v, int shift);
  void SubWord(std::vector<uchar> &v);
  void RCon(std::vector<uchar> &v, int rIndex);
...
namespace out
{
  ...
  void display_block(const uchar* block, size_t block_len);
}
